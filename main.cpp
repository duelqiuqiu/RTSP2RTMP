...
int main(int argc,const char** argv){
  ...
  avformat_alloc_output_context2(&output_format, nullptr, "flv", rtmp_url.c_str());
  assert(output_format != nullptr);
  
  for (int i = 0; i < format_ctx->nb_streams; i ++) {
    auto out_stream = avformat_new_stream(output_format, NULL);
    assert(out_stream != nullptr);
    
    auto in_stream = format_ctx->streams[i];
  ...
  while (true) {
    ...
    if(in_stream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO){
      auto time_base = in_stream->time_base;
      auto time_base_q = AV_TIME_BASE_Q;
      auto pts_time = av_rescale_q(packet.dts, time_base, time_base_q);
